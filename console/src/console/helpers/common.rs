...
pub fn rotate_operand<DIR: BitFlag, C: BitFlag>(r8: u8, console: &mut Console) {
    ...
    let mut reg: u8;
    if r8 != EA { reg = console[Byte { idx: r8 }]; }
    else { reg = console[Byte { idx: A }]; }
    ...
    console.clear_or_set_flag(reg == 0 && r8 != EA, flag::Z);
    console.clear_or_set_flag(c != 0, flag::C);

    if r8 != EA { *(&mut console[Byte { idx: r8 }]) = reg; }
    else { *(&mut console[Byte { idx: A }]) = reg; }
}